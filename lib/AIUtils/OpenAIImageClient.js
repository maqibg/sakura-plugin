...
function parseApiError(res, body) {
  try {
    ...
    return err.error?.message || err.message || body
  } catch {
    return body
  }
}
...
export class OpenAIImageClient {
  constructor(config) {
    ...
    this.moderation = config.moderation || "auto"
  }
  ...
  async generateWithChat(prompt, imageUrls = [], options = {}) {
    const stream = options.stream !== false
    const messages = [{
      role: "system",
      content: "You are an image generation assistant. Generate the requested image and return it directly.",
    }]
    ...
    if (stream) {
      return this._fetchStream("chat/completions", body)
    }

    const res = await this._fetch("chat/completions", body)
    return this._extractChatImages(res)
  }

  async _fetchStream(path, body) {
    ...
    try {
      ...
      return { stream: res.body, controller }
    } finally {
      clearTimeout(timeoutId)
    }
  }

  _extractChatImages(payload) {
    ...
    if (images.length === 0) {
      const text = choices.map(c =>
        Array.isArray(c.message?.content)
          ? c.message.content.filter(p => p.type === "text").map(p => p.text).join("")
          : c.message?.content
      ).filter(Boolean).join("\n")
      if (text) return [{ text }]
  ...
  _normalize(opts) {
    return {
      ...
      moderation: opts.moderation || this.moderation,
    }
  }

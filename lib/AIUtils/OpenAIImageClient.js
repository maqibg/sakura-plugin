...
export class OpenAIImageClient {
  ...
  async generate(prompt, options = {}) {
    const { size, quality, n, outputFormat } = this._normalizeOptions(options)
    const body = { model: this.model, prompt, size, quality, n, output_format: outputFormat }
  ...
  async generateWithResponses(prompt, imageUrls = [], options = {}) {
    const { size, quality, outputFormat } = this._normalizeOptions(options)
  ...
  _extractResponsesImages(payload) {
    ...
    for (const item of output) {
      ...
      if (typeof item.result === "string" && item.result.trim()) {
        images.push({ base64: item.result })
      }
    }

...
export class JMComicTool {
  name = "JMComic"
  description = "下载禁漫天堂漫画为PDF。传入车牌号（漫画ID）即可，下载完成后会返回PDF路径。拿到路径后请调用 sendForwardMessage 工具，以合并转发消息发送该文件，不携带group_id，senderName传入\"JM\",senderId传入车牌号。"
  parameters = {
    type: "object",
    properties: {
      id: {
  ...
  func = async (opts, e) => {
    ...
    return new Promise((resolve) => {
      exec(cmd, { timeout: 600000, maxBuffer: 1024 * 1024 }, (err, stdout) => {
        ...
        if (pdfPath) {
          resolve(`PDF已生成：${pdfPath}。请调用 sendForwardMessage 工具，传入 file 为该路径，以合并转发消息发送，不携带group_id，senderName传入\"JM\",senderId传入车牌号。`)
        } else {

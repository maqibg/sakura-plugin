...
import { bufferToFile } from "../../utils.js"
...
export class MarkdownTool extends AbstractTool {
  ...
  parameters = {
    properties: {
      markdownContent: {
        type: "string",
        description:
          "用于最终呈现给用户的、已经组织好的完整Markdown文本",
      },
    },
    required: ["markdownContent"],
  }
  description =
    "仅当需要展示标题、列表、要点等结构化内容时使用。在使用Search工具后，只有当整理的信息量较大且需要结构化展示时才使用此工具。"
  ...
  func = async function (opts, e) {
    ...
    try {
      ...
      await e.reply(segment.image(await bufferToFile(Buffer.from(imageBase64, "base64"))))
